    ...
    {   
        ...
        //$store_id = '5';
         $store=  DB::table('store')
               ...
               ->select('store.store_name','store.phone_number','store.email','store.city','store.store_state as state','store.address','store.zip_code','store.store_desc as category','store_earning.paid',DB::raw('SUM(orders.total_price)-SUM(orders.total_price)*(store.admin_share)/100 as store_earning'))
        ...
        if(!$store){
            $store=  DB::table('store')
               ->leftJoin('store_earning','store.store_id','=','store_earning.store_id')
               ->select('store.store_name','store.phone_number','store.email','store.city','store.store_state as state','store.address','store.zip_code','store.store_desc as category','store_earning.paid')
               ->groupBy('store.store_name','store.phone_number','store.email','store.address','store_earning.paid','store.admin_share')
                ->where('store.store_id', $store_id )
                ->first();
            $store->store_earning=0;    
        }        
