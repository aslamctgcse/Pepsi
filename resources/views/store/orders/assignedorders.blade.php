  ...
  <table class="table">
    ...
    <tbody>
          ...
          <tr>
            ...
            @if($ords->order_status == 'Confirmed'||$ords->order_status == 'confirmed'||$ords->order_status == 'Confirm'||$ords->order_status == 'confirm')
                 <a href="{{route('store_confirm_order', $ords->cart_id)}}" data-toggle="modal" data-target="#exampleModal1{{$ords->sub_order_id}}" rel="tooltip" class="btn btn-success reassignBtn">
                ReAssign DBoy
                </a>
            @endif
                      ...
                      @if(count($details)>0)
                        ...
                        @foreach($details as $detailss)
                          @if($detailss->cart_id==$ords->cart_id &&  $detailss->sub_order_cart_id==$ords->sub_order_cart_id)
                            <td><p><img style="width:25px;height:25px; border-radius:50%" src="@if(isset($detailss->varient_image)) {{url('/admin-panel/'.$detailss->varient_image)}} @endif" alt="$detailss->product_name">  {{$detailss->product_name}}({{$detailss->quantity}}{{$detailss->unit}})</p>
                            </td>
                            <td>{{$detailss->qty}}</td>
                            <td> 
