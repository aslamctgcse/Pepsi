...
class DeliveryController extends Controller
{
    ...
    public function location_track(Request $request)
    {
         $dboy_id = $request->id;
         //$dboy_id = 5;
         $title = "Delivery Boy Location Track";
         $admin_email=Session::get('bamaAdmin');
         $admin= DB::table('admin')
                   ->where('admin_email',$admin_email)
                   ->first();
        ...
        $d_boy=  DB::table('delivery_boy')
            ->where('dboy_id', $dboy_id)
            ->first();
        $d_boy_tack=  DB::table('rider_current_location')
            ->where('loc_dboy_id', $dboy_id)
            ->orderBy('loc_id', 'desc')
            ->first();
        //dd($d_boy_tack);        
        $city =DB::table('city')
        ...
        $map1 = DB::table('map_api')
         ...
         //$key = $map1->map_api_key;
			//$c =  $response = json_decode(file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?latlng=28.1139993,82.2896254&key=".$map));
        // dd($c);
		 if($d_boy_tack){
            if($d_boy_tack->loc_long && $d_boy_tack->loc_lat){
                $response = json_decode(file_get_contents("https://maps.googleapis.com/maps/api/geocode/json?latlng=".$d_boy_tack->loc_lat.",".$d_boy_tack->loc_long."&key=".$map));
                $d_boy_tack->address=$response->results[0]->formatted_address;
            }else{
                $d_boy_tack->address='';
            }
        ...
        $stores  =   DB::table('store')
                      ->where('store_id', $d_boy->store_id)
                      ->first();            

        return view('admin.d_boy.location',compact("d_boy","admin_email","admin","logo","title","city","map", 'stores','d_boy_tack'));
    }
